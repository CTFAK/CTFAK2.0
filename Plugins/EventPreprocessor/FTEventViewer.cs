...
public class FTEventViewer : IFusionTool
{
    ...
    public Dictionary<int, Dictionary<int, ActionHandler>> actionHandlers = new();

    public Dictionary<int, Dictionary<int, ConditionHandler>> conditionHandlers = new();
    public int[] Progress = { };
    ...
    public void Execute(IFileReader reader)
    {
        ...
        foreach (var type in types)
            if (type.IsSubclassOf(typeof(ExtensionHandler)))
            {
                ...
                foreach (var extension in game.Extensions.Items)
                    ...
                    {
                        Logger.Log(
                            $"Adding handlers for extension {extension.Name} with handle {extension.Handle + 32}");
                        newInstance.Init();
        ...
        var gotValue = int.TryParse(Console.ReadLine(), out var selectedIndex);
    ...
    {
        foreach (var eventGroup in frame.Events.Items) ProcessEventGroup(eventGroup);
    }
    ...
    public void ProcessEventGroup(EventGroup eventGroup)
    {
        WriteLine("IF:", 0);
        WriteLine("{", 0);
        foreach (var condition in eventGroup.Conditions) ProcessCondition(condition);
        WriteLine("}", 0);
        WriteLine("DO:", 0);
        WriteLine("{", 0);
        foreach (var action in eventGroup.Actions) ProcessAction(action);
        WriteLine("}", 0);
    }

    public void ProcessCondition(Condition condition)
    {
        Dictionary<int, ConditionHandler> currentHandlers = new();
        ...
        if (currentHandlers != null)
        {
            ...
            else
            {
                ...
                WriteLine("Params: ");
                foreach (var param in condition.Items) WriteLine($"Loader: {param.Loader}, Value: {param.Value}");
                //Console.ReadKey();
            }
        }
        else
        {
            WriteLine("CRITICAL IMPLEMENTATION ERROR");
        }
    }

    public void ProcessAction(Action condition)
    {
        Dictionary<int, ActionHandler> currentHandlers = new();
        var conditionObject = objects[condition.ObjectInfo];
            ...
            else
            {
                ...
                foreach (var param in condition.Items)
                    if (param.Loader is ExpressionParameter)
                        WriteLine(
                            $"Loader: {ExpressionConverter.ConvertExpression(param.Loader as ExpressionParameter)}");
                    else
