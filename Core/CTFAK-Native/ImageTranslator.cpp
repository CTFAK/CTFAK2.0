...
int GetPadding(int width, int pointSize, int bytes = 2)
{
    ...
    return ceil(static_cast<float>(pad) / static_cast<float>(pointSize));
}
...
DllExport void ReadPoint(char* result, int width, int height, int alpha, int size, char* imageData,
...
DllExport void ReadPointBGRA(char* result, int width, int height, int alpha, int size, char* imageData,
                             int transparentColor)
{
    int stride = width * 4;
    int pad = GetPadding(width, 3);
...
DllExport void ReadFifteen(char* result, int width, int height, int alpha, int size, char* imageData,
                           int transparentColor)
{
    int stride = width * 4;
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            UINT16 newShort = static_cast<UINT16>(imageData[position] | imageData[position + 1] << 8);
            byte r = static_cast<byte>((newShort & 31744) >> 10);
            byte g = static_cast<byte>((newShort & 992) >> 5);
            byte b = static_cast<byte>((newShort & 31));

            r = static_cast<byte>(r << 3);
            g = static_cast<byte>(g << 3);
...
DllExport void ReadSixteen(char* result, int width, int height, int alpha, int size, char* imageData,
                           int transparentColor)
{
    int stride = width * 4;
    int pad = GetPadding(width, 2);
    ...
    for (int y = 0; y < height; y++)
    {
        ...
        {
            UINT16 newShort = static_cast<UINT16>(imageData[position] | imageData[position + 1] << 8);
            byte r = static_cast<byte>((newShort & 63488) >> 11);
            byte g = static_cast<byte>((newShort & 2016) >> 5);
            byte b = static_cast<byte>((newShort & 31));

            r = static_cast<byte>(r << 3);
            g = static_cast<byte>(g << 2);
            b = static_cast<byte>(b << 3);
            result[(y * stride) + (x * 4) + 2] = r;
            result[(y * stride) + (x * 4) + 1] = g;
            result[(y * stride) + (x * 4) + 0] = b;
...
DllExport void TranslateToRGBA(char* result, int width, int height, int alpha, int size, char* imageData,
                               int transparentColor, int colorMode)
{
    ReadPoint(result, width, height, alpha, size, imageData, transparentColor);
}

DllExport void TranslateToBGRA(char* result, int width, int height, int alpha, int size, char* imageData,
                               int transparentColor, int colorMode)
{
